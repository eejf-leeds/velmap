...
for i=1:nvtx
  ...
  %JF testing
%   if ~issymmetric(vcm_tmp)
%     warning('VCM is not symmetric; symmetrizing...')
%     vcm_tmp = (vcm_tmp + vcm_tmp.') / 2;
%   end
%   eigVals = eig(vcm_tmp);
%   epsilon = 1e-8;  % small regularization term
%   vcm_tmp = vcm_tmp + (abs(min(eigVals)) + epsilon) * eye(size(vcm_tmp));  % regularize
  

 [m(i,:),~,mse,covm(i,:,:)]=lscov(G,dvel,vcm_tmp); %orig
  %covm(i,:,:)=covm(i,:,:)/mse;
